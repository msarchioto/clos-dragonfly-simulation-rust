...
pub fn generate(
    ...
    let (a, h, p, g) = find_best_config(ports, lph, num_hosts);
    ...
    // Sort for deterministic output matching Python/Go
    links.sort();
...
fn find_best_config(k: u32, lph: u32, num_hosts: u32) -> (u32, u32, u32, u32) {
    let mut best = (0, 0, 0, 0);
    let mut best_routers = u32::MAX;
    ...
    for h in 1..k {
        for a in 2..k {
            ...
            for g in 2..=g_max {
                ...
                if total_r < best_routers {
                    update = true;
                } else if total_r == best_routers {
                    if imbalance < best_imbalance {
                        update = true;
                    } else if imbalance == best_imbalance && slack < best_slack {
                        update = true;
                    }
                }
                if update {
                    best = (a, h, p, g);
                    best_routers = total_r;
    ...
    if best.0 == 0 {
        panic!("No valid Dragonfly config for {} hosts", num_hosts);
    }
    best
}
...
fn wire_global_links(a: u32, g: u32, h: u32) -> Vec<(u32, u32)> {
    ...
    if rem.iter().any(|&v| v != 0) {
        panic!("Failed to satisfy group degree");
    }
